...
class OpTable:
    ...
    def GetBollList(self, currency_type, duration, fetch_size=20, now_id=0):
        ...
        data = orm.session.query(schema.Boll).filter(schema.Boll.currency_type == currency_type,
                                                     ...
                                                     schema.Boll.id <= now_id).order_by(schema.Boll.id.desc()).limit(
            fetch_size)
        orm.session.commit()
        length = len(data)
        begin = data[0].id
        ...
        if length != (end - begin) / interval + 1:
            log_error(
                "boll data is not lost. begin=%d||end=%d||duration=%s||length=%d" % (begin, end, duration, length))
            data.clear()
        return data
    ...
    def GetMAList(self, currency_type, duration, fetch_size, now_id=0):
        ...
        data = orm.session.query(schema.MaInfo).filter(schema.MaInfo.currency_type == currency_type,
                                                       ...
                                                       schema.MaInfo.id <= now_id).order_by(
            schema.MaInfo.id.desc()).limit(fetch_size)
        orm.session.commit()
        length = len(data)
        begin = data[0].id
        end = data[length - 1].id
    ...
    def GetPriceList(self, currency_type, duration, fetch_size, now_id=0):
        ...
        price_info_lines = orm.session.query(schema.PriceInfo).filter(schema.PriceInfo.currency_type == currency_type,
                                                                      ...
                                                                      schema.price_list.id <= now_id).order_by(
            schema.PriceInfo.id.desc()).limit(fetch_size).all()
        orm.session.commit()
        ...
        if real_size != (end - begin) / interval + 1:
            log_error("price data is lost. begin=%d||end=%d||currency_type=%s||period=%s"(begin, end, currency_type,
                                                                                          duration))
    ...
    def StoreBoll(self, currency_type, duration):
        ...
        latest_status1_line = orm.session.query(schema.BollInfo).filter(schema.PriceInfo.currency_type == currency_type,
                                                                        schema.PriceInfo.period == duration,
                                                                        schema.PriceInfo.status == 1).order_by(
            schema.BollInfo.id.desc()).limit(1).first()
        orm.session.commit()
        status1_latest_id = latest_status1_line
        now = time.time()
        to_fill_size = int((now - status1_latest_id) / interval)

        latest_status0_lines = orm.session.query(schema.BollInfo).filter(
        ...
        if len(price_list) != fetch_size:
            log_warn(
                "UpdateBoll no enough data in table %s of currency_type=%s || step=%d. get %d items while %d items is expected. from %s to %s" % (
                    ptable, currency_type, step, len(price_list), fetch_size, timestamp2string(price_list[0].id),
                    timestamp2string(price_list[-1].id)));
    ...
    def StoreMa(self, currency_type, duration, refresh=True):
        ...
        latest_line = orm.session.query(schema.MaInfo).filter(schema.MaInfo.currency_type == currency_type,
                                                              ...
                                                              schema.MaInfo.status == 1).order_by(
            schema.MaInfo.id.desc()).first()
        orm.session.commit()
        latest_id = 0
        ...
        if len(total_price_list) != fetch_size:
            log_warn("enough data in table %s of currency_type=%s . get %d items while %d items is expected" % (
                ptable, currency_type, len(total_price_list), fetch_size));
        ...
        for step in Slist:
            ...
            orm.session.commit()
            log_info("StoreMa.  update_num=%d || add_num=%d || MA=%d || currency_type=%s || duration=%s" % (
                update_num, add_num, step, currency_type, duration))
    ...
    def GetLSRatio(self, contract_type, currency_type, market, ts_end=0, period=var.Day):
        ...
        latestLsRatioLines = orm.session.query(schema.LongShortRatio).filter(schema.LongShortRatio.market == market,
                                                                             ...
                                                                             schema.LongShortRatio.id % 1800 == 0).order_by(
            schema.LongShortRatio.id.desc()).all()
        orm.session.commit()
    ...
    def StoreInterestVolume(self, currency_type, size=48, period=var.Hour, refresh=False):
        ...
        for contract_type in var.ContractTypes:
            ...
            latestLsRatioLines = orm.session.query(schema.LongShortRatio).filter(
                schema.LongShortRatio.market == 'Huobi',
                schema.LongShortRatio.contract_type == contract_type,
                schema.LongShortRatio.currency_type == currency_type,
                (schema.LongShortRatio.id + 8 * 3600) % align_seconds == 0,
                schema.LongShortRatio.id >= ago,
                schema.LongShortRatio.amount_volume < min_volume).order_by(schema.LongShortRatio.id.desc()).limit(
                2000).all()
            orm.session.commit()
            to_update_cnt = len(latestLsRatioLines)
            if to_update_cnt != 0:
                log_info("going to store long short ratio. currency_type=%s || contract_type=%s || to_update_cnt=%d" % (
                    currency_type, contract_type, to_update_cnt))
            else:
                log_info("no need to update lsr volume . currency_type=%s || contract_type=%s || to_update_cnt=%d" % (
                    currency_type, contract_type, to_update_cnt))
                continue
            id_volumes = {}
            ...
            for tradeInfo in tradeInfoList:
                if tradeInfo.id_ in id_volumes and int(tradeInfo.volume_) != int(id_volumes[tradeInfo.id_]):
                    ...
                    orm.session.query(schema.LongShortRatio).filter(schema.LongShortRatio.market == 'Huobi',
                                                                    schema.LongShortRatio.contract_type == contract_type,
                                                                    schema.LongShortRatio.currency_type == currency_type,
                                                                    schema.LongShortRatio.id == tradeInfo.id_).update(
                        {"amount_volume": tradeInfo.volume_})
    ...
    def StoreLongShortRatio(self, currency_type, period=Hour):
        ...
        for contract_type in var.ContractTypes:
            ...
            latestLsRatioLines = orm.session.query(schema.LongShortRatio).filter(
                ...
                schema.LongShortRatio.id % align_seconds == 0).order_by(schema.LongShortRatio.id.desc()).limit(
                300).all()
            orm.session.commit()
            ...
            log_info("exists key for contract_type=%s, market=%s, currency_type=%s, period=%s. [%s]" % (
                contract_type, market, currency_type, period, contains))
            ...
            for ls_type in var.LsTypes:
                ...
                for bsInfo in buySellList:
                    ...
                    if int(bsInfo.id_) % align_seconds != 0:
                        # 时间序列未align
                        log_error("duration is not align to %d, with id: %d(%s)", align_seconds, bsInfo.id_,
                                  timestamp2string(bsInfo.id_))
                        continue
    ...
    def StorePrice(self, currency_type, period, size=100):
        ...
        latest_valid_line = orm.session.query(schema.PriceInfo).filter(schema.PriceInfo.period == period,
                                                                       schema.PriceInfo.currency_type == currency_type,
                                                                       schema.PriceInfo.status == 1).order_by(
            schema.PriceInfo.id.desc()).first()
        orm.session.commit()
        latest_id = 0
        ...
        zero_status_record = orm.session.query(schema.PriceInfo).filter(schema.PriceInfo.period == period,
                                                                        schema.PriceInfo.currency_type == currency_type,
                                                                        schema.PriceInfo.status == 0).order_by(
            schema.PriceInfo.id.desc()).all()
        orm.session.commit()
        zero_status_set = {0}
        ...
        log_info("UpdatePrice begin: to_update_num=%d || currency_type=%s || period=%s " % (
            missed_number, currency_type, period))
        ...
        for i in range(0, len(price_list)):
            ...
            ts = kl.id_
            if i % 100 == 0:
                log_info("update price data for currency_type=%s || duration=%s || ts=%s" % (
                    currency_type, period, timestamp2string(ts)))
            this_status = 0
            if now - ts > interval:
                this_status = 1
            ...
            if ts in zero_status_set:
                ...
                orm.session.query(schema.PriceInfo).filter(schema.PriceInfo.currency_type == currency_type,
                                                           ...
                                                           schema.PriceInfo.period == period).update(
                    {"status": this_status, "high": kl.high_, "low": kl.low_, "close": kl.close_, "volume": kl.amount_})
                orm.session.commit()
            else:
                add_num = add_num + 1
                orm.session.add(price_info)
        orm.session.commit()
        log_info("UpdatePrice end: add_num=%d || update_num=%d || currency_type=%s || period=%s " % (
            add_num, update_num, currency_type, period))
        return
